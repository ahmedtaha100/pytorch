...
class SubprocPool:
    ...
    def _decode_job_result(
        self, job_id: int, data: bytes, *, failed: bool
    ) -> object:
    ...
    @staticmethod
    def _run_job_bookkeeping(
        callback: Callable[[], object], description: str
    ) -> None:
        try:
            callback()
        except BaseException:
...
class SubprocMain:
    ...
    def _result_payload(
        self, job_id: int, fut: Future[Any]
    ...
    def _send_result(
        self, job_id: int, msg_header: MsgHeader, payload: bytes
    ) -> None:
        try:
            with self.write_lock:
                if not self.running:
