...
class TestArchRanges(TestCase):
    ...
    def test_sm_predicates_match_their_names(self):
        for name, rng in cc.GATE_ARCH_RANGES.items():
            if not name.startswith(("SM", "IS_SM")):
                continue
            digits = "".join(ch for ch in name if ch.isdigit())
            expected_lo = (int(digits[:-1]), int(digits[-1]))
            self.assertEqual(
                rng[0],
                expected_lo,
                f"{name} declares floor {rng[0]}, name says {expected_lo}",
            )
            self.assertEqual(
                rng[1] is None,
                name.endswith("OrLater"),
                f"{name}: only *OrLater predicates may be unbounded",
            )
